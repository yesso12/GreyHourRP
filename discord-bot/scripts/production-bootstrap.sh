...
log "INFO" "step 1/6: preflight checks"
npm run preflight
pass "preflight checks passed"

log "INFO" "step 2/6: register discord slash commands"
npm run register
pass "slash commands registered"

log "INFO" "step 3/6: canary pre-smoke (must pass before restart)"
bash scripts/smoke-check.sh
pass "pre-smoke check passed"

log "INFO" "step 4/6: ensure service is enabled (idempotent)"
...
log "INFO" "step 5/6: restart service"
...
log "INFO" "step 6/6: canary post-smoke"
bash scripts/smoke-check.sh
